...
|[Linked Places](https://github.com/LinkedPasts/linked-places)|[DicoTopo API](https://dev.chartes.psl.eu/dico-topo/api/1.0/placenames/DT02-01486?include=old-labels,commune,localization-commune,feature-types)|Comments|
|---|---|---|
|`$.features.@id`|`$.data.links.self`|String processing required for conversion if link to page and not to the resource (API).|
...
|`$.features.geometry.geometries[?(@.type='Point')].src`|`$.included[?(@.type='commune')].attributes.insee-code`|No insee URI; build `insee:insee-code` (eg `insee:02196`)?|
...
|`$.features.types.identifier`|not available (yet)|TODO: processing to extract feature type + API refactoring to get feature type|
...
|`$.features.relations.relationType`|[`"gvp:broaderPartitive"`](http://vocab.getty.edu/ontology#broaderPartitive)|Only the parent? For Clacy, we can rebuild: Clacy < canton de Laon-1 (`canton`) < arrondissement de Laon (`arrondissement`) < canton Aisne (`département`) < Picardie (`région`) < France (`pays`)<br/><ul><li>For `commune` (town) : easy to get the `département` (probably the most valuable data for disambiguating).</li><li>For inframunicipal place : easy to get (1) the `commune` and (2) the `département`.</li>.</ul>|
|`$.features.relations.relationTo `|?|Do we need to declare a URI for each administrative entity? Or can we link to another source such as IGN or wikidata?|
|`$.features.relations.label`|?|TODO: provide the ability to build this label (`Commune de…`, `Département de…`, `Région…`)|
|`$.features.relations.when.start`|not available|TODO: get the year of creation of each French *département* and *région* and store the data.|
|`$.features.relations.when.end`|none|Can we omit it? (eg `clacy_lp.json`)|
|`$features.links`|not available|**TODO (top priority)**: link insee codes with geonames URIs.|
|`$features.descriptions.@id`|URL of DT entity ?|Pocessing required. Correct semantics?|
|`$features.descriptions.value`|`$data.attributes.desc`|Correct semantics?|
|`$features.descriptions.lang`|`"fr"`||
|`$features.depictions`|not available||


Référentiel -> on essaye de maintenir le notre, pas de réfzrentiel cible en vue à ce stade
Relations: ajouter les lieux rattachés à la même commune

## `clacy_lp.json`
DT source (API): https://dev.chartes.psl.eu/dico-topo/api/1.0/placenames/DT02-01486?include=old-labels,commune,localization-commune,feature-types
